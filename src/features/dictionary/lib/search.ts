...
import { hasKannada, latinToKannada, normalise, phoneticKey, shardKey } from "@/lib/kannada";
...
/** Kannada query: exact → prefix → phonetic, all within the query's shard. */
async function searchKannada(q: string): Promise<SearchResult[]> {
  const shard = await loadShard(shardKey(q));
  if (!shard) return [];
  ...
  for (const e of shard.entries) if (e.key.startsWith(key)) push(e, "phonetic");
  return out;
}
